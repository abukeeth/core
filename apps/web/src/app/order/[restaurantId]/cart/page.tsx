          ...
          {cart.items.map((item) => (
            <li key={item.id} className="flex items-center justify-between gap-4 p-4">
              <div className="flex flex-col gap-1">
                <span className="font-medium text-ink">{cartItemLabel(item)}</span>
                {item.modifiersSnapshot?.modifiers && item.modifiersSnapshot.modifiers.length > 0 && (
                  <span className="text-xs text-ink-secondary">
                    {item.modifiersSnapshot.modifiers.map((m) => m.optionName).join(" · ")}
                  </span>
                )}

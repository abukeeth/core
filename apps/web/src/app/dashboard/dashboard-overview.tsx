...
const NAV_ITEMS: Array<[string, string, IconName]> = [
  ...
  ["Storefront Studio", "/dashboard/website", "website"], ["Customers", "/dashboard/customers", "customers"], ["Marketing", "/dashboard/marketing", "marketing"],
...
export function DashboardOverview({userName}:{userName:string}){
  ...
  const maxRevenue=Math.max(1,...byDay.map(i=>i.revenueCents)); const insight=topItems[0]?`${topItems[0].name} is your leading item this week with ${topItems[0].quantitySold} sold and ${money(topItems[0].revenueCents)} in revenue.`:"Your business insights will appear here once you have enough order activity.";
  return <div className="min-h-screen bg-[#F7F0E5] text-[#171512] lg:grid lg:grid-cols-[240px_1fr]">
    ...
    <main className="min-w-0 overflow-x-hidden px-4 pb-28 pt-5 sm:px-6 lg:p-10">
      ...
      <section className="mt-4 grid gap-4 xl:grid-cols-[0.95fr_1.05fr]"><div className="rounded-3xl bg-[#171512] p-6 text-white shadow-xl shadow-black/10"><div className="flex items-center justify-between gap-3"><h2 className="text-xl font-bold text-[#E1B56F]">Business Copilot</h2><span className="rounded-full bg-white/10 px-3 py-1 text-xs font-semibold text-[#E9E0D4]">Insight</span></div><p className="mt-5 max-w-xl text-base leading-7 text-[#E9E0D4]">{insight}</p><Link href="/dashboard/analytics" className="mt-6 inline-flex items-center gap-2 rounded-2xl bg-[#B97824] px-4 py-3 text-sm font-semibold text-white">Explore analytics <Icon name="arrow" className="h-4 w-4"/></Link></div>

...
describe("serverFetch — timeout and network error mapping", () => {
  ...
  it("strips a trailing slash from API_URL so the request never has a double slash", async () => {
    ...
    try {
      ...
      // The join between host and path must be a single slash — the bug was
      // an "https://host//api/..." double slash (ignoring the scheme's own).

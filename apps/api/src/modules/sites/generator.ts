...
class InProcessGenerationJobRunner implements GenerationJobRunner {
  ...
  private async run(jobId: string, siteId: string, batchId: string, createdById: string): Promise<void> {
    ...
    try {
      ...
      // Sprint 5.5 — Brand stage. One Brand Kit per business (deterministic
      // palette + vocabulary even with AI text off), shared across all three
      // variations so color/identity is consistent. Impression images (hero /
      // category / marketing) are generated once here — flag-gated, capped, and

...
describe("§Website Builder — the 3 design systems render materially differently", () => {
  it("registers exactly modern-editorial, warm-local, and bold-commerce as the active design systems", () => {
    expect(ACTIVE_THEMES.map((t) => t.key).sort()).toEqual(["bold-commerce", "modern-editorial", "warm-local"]);
  });
